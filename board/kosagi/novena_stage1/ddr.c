				...
				uint32_t *dram_sdqs_pad_ctls)
{
	...
	/*
	printf("Hard-dump registers\n" );
	printf("pad_ctl_sdclk0_p %08x\n", *((unsigned int *) 0x20e04ac));
	printf("pad_ctl_sdclk1_p %08x\n", *((unsigned int *) 0x20e04b0));
	printf("ddr_type %08x\n", *((unsigned int *) 0x20e0774));
	printf("debug register 0 %08x\n", *((unsigned int *) 0x21b0410));
	printf("debug register 1 %08x\n", *((unsigned int *) 0x21b4410));
	printf("mmdc control register %08x\n", *((unsigned int *) 0x21b0000));
	printf("mmdc pd register %08x\n", *((unsigned int *) 0x21b0004));
	printf("mmdc odt register %08x\n", *((unsigned int *) 0x21b0008));
	printf("mmdc core cfg 0 register %08x\n", *((unsigned int *) 0x21b000c));
	printf("mmdc core cfg 1 register %08x\n", *((unsigned int *) 0x21b0010));
	printf("mmdc core cfg 2 register %08x\n", *((unsigned int *) 0x21b0014));
	printf("mmdc core misc register %08x\n", *((unsigned int *) 0x21b0018));

	printf("ccm_analog_misc_2 %08x\n", *((unsigned int *) 0x20c8170));
	printf("pmu_reg_core %08x\n", *((unsigned int *) 0x20c8140));
	printf( "\n\nCCM_CBCDR: %08x\n", *((unsigned int *) 0x20c4014));  // 0x18d00
	// mmdc_ch0_axi_podf = div by 1; periph_clk_sel = 0 which is from pll2_sw_clk muxed clock source
	//  CBCDR: periph_clk_sel | PLL_bypass_en2 (jtag) -> clock gate ->  CBCDR: mmdc_ch0_axi_podf

	printf( "CCM_CBCMR: %08x\n\n", *((unsigned int *) 0x20c4018));  // 0x60324  -- not default, points to 396 MHz PFD2
	
	printf( "CCM_CCGR3: %08x\n\n", *((unsigned int *) 0x20c4074));  // 0x3ff0_0000
	
	printf( "MMDC1 AXI reordering: %08x\n", *((unsigned int *)0x21b0400) );
	printf( "MMDC2 AXI reordering: %08x\n", *((unsigned int *)0x21b4400) );
	*/

	/*	printf( "manually stuff\n" );
	...
	//	*(unsigned int *)0x20c4080 = 0xfc3;

	//	for( i = 0; i < 0x2000; i += 4) {
	//	  if( i % 32 == 0 )
	//	    printf( "\n%06x: ", i + 0x91F000 );
	//  printf( "%08x ", *((unsigned int *) (i + 0x91F000)) );
	// }
	//	printf("nic-301 gpv0 %08x\n", *((unsigned int *) 0x0b000000) );
	//	printf("set gpv0\n" );
	//	*((unsigned int *) 0x0b000000) = 0x00;
	//	printf("nic-301 gpv0 %08x\n", *((unsigned int *) 0x0b000000) );
	
	printf( "manually stuff EIM\n" );
	*((unsigned int *) 0x08000008) = *((unsigned int *) 0x08000000);
	printf( "manually stuff DDR\n" );
	*((unsigned int *) 0x10000008) = *((unsigned int *) 0x10000000);
	*/

	printf("Write/read test before SPD reading\n");
	write_random_data(0);
	read_random_data(0);
...
extern struct i2c_pads_info i2c_pad_info0_6q;
extern struct i2c_pads_info i2c_pad_info0_6dl;
...
uint32_t imx6dl_ddr_init(void)
{
	...
	static iomux_v3_cfg_t const unused_i2c1_pins[] = {
		...
		//MX6DL_PAD_CSI0_DAT9__GPIO5_IO27 | MUX_PAD_CTRL(NO_PAD_CTRL),
		//MX6DL_PAD_CSI0_DAT8__GPIO5_IO26 | MUX_PAD_CTRL(NO_PAD_CTRL),
	};
	imx_iomux_v3_setup_multiple_pads(unused_i2c1_pins,
					ARRAY_SIZE(unused_i2c1_pins));
	gpio_direction_input(IMX_GPIO_NR(3, 28));
	...
	//gpio_direction_input(IMX_GPIO_NR(5, 27));
	//gpio_direction_input(IMX_GPIO_NR(5, 26));
...
int dram_init(void)
{
	...
	printf( "%08x - GPR5 iomuxc\n", *((unsigned int *) 0x20e0014 ) );
	printf( "%08x - GPR6 iomuxc\n", *((unsigned int *) 0x20e0018 ) );

	printf( "%08x - GPR9 iomuxc\n", *((unsigned int *) 0x20e0024 ) );
	*((unsigned int *) 0x20e0024 ) = 0x3;
	printf( "%08x - GPR9 iomuxc\n", *((unsigned int *) 0x20e0024 ) );
	printf( "%08x - GPR12 iomuxc\n", *((unsigned int *) 0x20e0030 ) );

	printf( "%08x - GPR3 iomuxc\n", *((unsigned int *) 0x20e000c ) );
	*((unsigned int *) 0x20e000c) = 0x01e01800;
	printf( "%08x - GPR3 iomuxc\n", *((unsigned int *) 0x20e000c ) );
	*/
